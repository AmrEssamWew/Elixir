defmodule Game_server do
 use GenServer

 def init({1,stones_num}) do
  IO.puts("\n it's player 1 turn and The total number of stones is #{stones_num}\n")
  {:ok,{1,stones_num}}
 end
 def start(stones_num\\30),do: GenServer.start(__MODULE__,{1,stones_num},name: __MODULE__)

 def take(stones_to_take),do: GenServer.call(__MODULE__,{:take,stones_to_take})

 def handle_call({:take,stones_to_take}, _, {player,stones_num}) when stones_to_take==stones_num   do
  {:stop, :normal, {:Winner, "Player #{next(player)} is the winner"}, {1, 0}}
end

def handle_call({:take,stones_to_take}, _, {player,stones_num}) when stones_to_take not in 1..3 or stones_to_take>stones_num   do
  {:reply, {:error,"\nYou should take from 1 to 3 stones only and you can't take more stones than the total number of remaining stones\n"}, {player,stones_num}}
end

def handle_call({:take,stones_to_take}, _, {player,stones_num})   do
  new_stones_num=stones_num-stones_to_take
  nextplayer=next(player)
  {:reply,{:ok,"\it's player #{nextplayer} turn\n the remaining number of stones is #{new_stones_num}\n"},{nextplayer,new_stones_num}}
end
def next(1),do: 2
def next(2),do: 1
end
